...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  ...
  if (!response.text) {
    throw new Error('Não foi possivel gerar a transcição')
  }
...
export async function generateAnswer(
  ...
  const prompt = `
        Com base no texto fornecido como contexto, responda de forma clara e precisa em português do Brasil.
        ...
        - Se a resposta não for encontrada no contexto, apenas responda que não tem as informações suficientes para responder;
        - Seja objetivo;
